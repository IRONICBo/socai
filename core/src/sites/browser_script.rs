...
use crate::cdp::{PageJavascriptSession, PageSession};
...
static OVERRIDE_LIFECYCLE_LOCKS: OnceLock<StdMutex<HashMap<String, Arc<tokio::sync::Mutex<()>>>>> =
    OnceLock::new();

fn override_lifecycle_lock(site: &str, tool: &str) -> Arc<tokio::sync::Mutex<()>> {
    let locks = OVERRIDE_LIFECYCLE_LOCKS.get_or_init(|| StdMutex::new(HashMap::new()));
    let key = format!("{site}:{tool}");
    let mut locks = locks
        .lock()
...
/// Monitors the exact CDP page generation used by one browser-script run.
/// Browser-script JavaScript and trusted input actions can both initiate a
/// navigation, including redirects that are only known after commit. Polling
/// navigation history does not need a live JavaScript context, so an off-site
/// top frame can be stopped and restored even when the operation itself fails
/// because its execution context was destroyed.
struct SiteAuthorityGuard {
    stop: Option<tokio::sync::oneshot::Sender<()>>,
    monitor: Option<tokio::task::JoinHandle<Result<Option<String>>>>,
}

impl SiteAuthorityGuard {
    async fn arm(page: &Arc<PageSession>, site: &str) -> Result<Self> {
        let javascript_session = page.javascript_session().await;
        let fallback_url = javascript_session.top_frame_url().await?;
        if !site_url_allowed(site, &fallback_url) {
            anyhow::bail!(
                "browser_script refused page outside the configured {site} site: {fallback_url}"
            );
        }
        ...
        let monitor = tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = &mut stop_rx => {
                        enforce_site_authority(
                            &javascript_session,
                            &site,
                            &fallback_url,
                            &mut first_violation,
                        ).await?;
                        return Ok(first_violation);
                    }
                    _ = poll.tick() => {
                        enforce_site_authority(
                            &javascript_session,
    ...
    async fn finish(mut self) -> Result<()> {
        ...
        let violation = self
            .monitor
            .take()
            .expect("site authority monitor is present")
            .await
            .context("site authority monitor stopped unexpectedly")??;
        if let Some(url) = violation {
            anyhow::bail!(
                "browser_script blocked navigation outside the configured site and restored the original page: {url}"
            );
        }
        Ok(())
    }
...
impl Drop for SiteAuthorityGuard {
    fn drop(&mut self) {
        if let Some(stop) = self.stop.take() {
            let _ = stop.send(());
        }
        // Dropping the caller future (task cancellation) must not cancel the
        // monitor. It owns the immutable CDP generation and performs one final
        // authority check/restore before exiting.
...
async fn enforce_site_authority(
    session: &PageJavascriptSession,
    site: &str,
    fallback_url: &str,
    first_violation: &mut Option<String>,
) -> Result<()> {
    let current_url = session.top_frame_url().await?;
    if site_url_allowed(site, &current_url) {
        return Ok(());
    }
    if first_violation.is_none() {
        *first_violation = Some(current_url);
    }
    // The stop is best-effort because a just-committed navigation can already
    // have no pending loader. Restoring the known-good site URL is mandatory.
    let _ = session.stop_loading().await;
    session
        .navigate(fallback_url)
        .await
        .context("failed to restore the browser tab after blocked off-site navigation")
}
...
struct TargetEvaluationGuard {
    javascript_session: PageJavascriptSession,
    armed: bool,
}

impl TargetEvaluationGuard {
    fn new(javascript_session: PageJavascriptSession) -> Self {
        Self {
            javascript_session,
            armed: true,
        }
...
impl Drop for TargetEvaluationGuard {
    fn drop(&mut self) {
        ...
        let javascript_session = self.javascript_session.clone();
        if let Ok(runtime) = tokio::runtime::Handle::try_current() {
            runtime.spawn(async move {
                let _ = tokio::time::timeout(
                    Duration::from_secs(1),
                    javascript_session.terminate_javascript(),
                )
                .await;
            });
        }
...
impl BrowserScriptRuntime {
    ...
    async fn execute(&self, script: &str, input: &Value, timeout_ms: u64) -> Result<Value> {
        ...
        let authority = SiteAuthorityGuard::arm(&self.page, &self.site).await?;
        ...
        let mut value = self
            ...
            .await;
        if let Err(error) = authority.finish().await {
            value = Err(error);
        }
        if value.is_err() {
    ...
    async fn handle_bridge_request(
        ...
        self.ensure_site_authority(matches!(operation, "navigate" | "page_info"))
            .await?;
        let result = match operation {
            ...
            "navigate" => {
                ...
                if !site_url_allowed(&self.site, url) {
                    anyhow::bail!(
                        "browser-script navigation must stay on the configured {} site",
                        self.site
                    );
                }
        ...
        self.ensure_site_authority(operation == "page_info").await?;
        Ok(result)
    }
    ...
    async fn evaluate_target(
        ...
        self.ensure_site_authority(false).await?;
        let expression = bounded_page_expression(script, input)?;
        let javascript_session = self.page.javascript_session().await;
        let mut cancellation = TargetEvaluationGuard::new(javascript_session.clone());
        let value = javascript_session
        ...
        let value = value?;
        let value = decode_bounded_result(value)?;
        self.ensure_site_authority(false).await?;
        Ok(value)
    }
...
"#
    ...
    async fn ensure_site_authority(&self, allow_blank: bool) -> Result<Value> {
        let page = self.page.page_info().await?;
        let url = page.get("url").and_then(Value::as_str).unwrap_or_default();
        if site_url_allowed(&self.site, url) || (allow_blank && url == "about:blank") {
            return Ok(page);
        }
        anyhow::bail!(
            "browser_script refused page outside the configured {} site: {}",
            self.site,
...
impl Tool for BrowserScriptTool {
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult> {
        ...
        let save_tool = input
            ...
            .filter(|value| !value.is_empty());
        let lifecycle_lock =
            save_tool.map(|tool| override_lifecycle_lock(&self.runtime.site, tool));
            ...
            Err(failure) => {
                ...
                self.runtime
                    ...
                    .transition(
                        ...
                        "builtin_and_override_failed_after_version_change",
                        true,
                    )
                    .context(
                        "failed to quarantine a local override after upgrade revalidation failed",
                    )?;
                let error = format!(
...
impl Tool for LocalOverrideTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> Result<ToolResult> {
        let initial_override = match self
            .runtime
            .registry
        ...
        if initial_override.is_none() {
            return self.call_builtin(input, ctx).await;
        }

        // Ordinary built-in calls never take a lifecycle lock. Only a tool
        // with an active override serializes its own canary/transition flow;
        // other sites, tools, and independent tabs continue in parallel.
        let lifecycle_lock = override_lifecycle_lock(&self.runtime.site, self.inner.name());
...
fn site_url_allowed(site: &str, value: &str) -> bool {
