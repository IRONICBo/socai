...
mod tests {
    ...
    fn brief() -> ResearchBrief {
        ResearchBrief {
            objective: "Compare two topics".to_string(),
            deliverable: "A concise comparison".to_string(),
            scope: ResearchScope {
            ...
            subquestions: vec![
                ResearchSubquestion {
                    id: "Q1".to_string(),
                    question: "First topic?".to_string(),
                    priority: ResearchPriority::Required,
                    evidence_requirements: vec!["user reports".to_string()],
                },
                ResearchSubquestion {
                    id: "Q2".to_string(),
                    question: "Second topic?".to_string(),
                    priority: ResearchPriority::Required,
                    evidence_requirements: vec!["user reports".to_string()],
                },
            ],
            ...
            assumptions: Vec::new(),
            initial_search_angles: vec!["comparison".to_string()],
            stop_conditions: vec!["Both topics assessed".to_string()],
        }
    }

    fn missing_coverage(id: &str) -> Value {
        json!({
            "id": id,
            "status": "missing",
            "evidence_refs": [],
            ...
            "gap": {
                "kind": "limitation",
                "description": "No decisive evidence was available"
            }
        })
    }

    #[test]
    fn completion_subquestions_are_normalized_to_brief_order() {
        let submission = ResearchCompletionSubmission::from_tool_input(
            json!({
                "subquestions": [missing_coverage("Q2"), missing_coverage("Q1")],
                "hard_constraints_satisfied": false,
                "stop_conditions_satisfied": false,
                "unmet_requirements": ["More evidence is required"],
                "final_answer": "The available evidence is insufficient for a reliable comparison."
            }),
            &brief(),
            &EvidenceLocatorCatalog::default(),
        )
        .expect("reordered coverage should be accepted");

        assert_eq!(submission.subquestions[0].id, "Q1");
        assert_eq!(submission.subquestions[1].id, "Q2");
    }

    #[test]
    fn completion_subquestions_reject_duplicate_ids() {
        let error = ResearchCompletionSubmission::from_tool_input(
            json!({
                "subquestions": [missing_coverage("Q1"), missing_coverage("Q1")],
                "hard_constraints_satisfied": false,
                "stop_conditions_satisfied": false,
                "unmet_requirements": ["More evidence is required"],
                "final_answer": "The available evidence is insufficient for a reliable comparison."
        ...
        .expect_err("duplicate coverage ids must be rejected");

        assert!(error.to_string().contains("duplicate subquestion id Q1"));
    }
}
