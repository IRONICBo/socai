...
impl ResearchWorkflow {
    pub async fn prepare(ctx: ResearchPrepareContext<'_>) -> anyhow::Result<PrepareOutcome> {
        ...
        let mut workflow = Self {
            runtime: None,
            extra_instructions: ctx.extra_instructions.to_string(),
        };
                ...
                Ok(()) => {
                    if let Some(question) = envelope.clarification() {
                        ...
                        match brief.execution_prompt() {
                            Ok(prompt) => {
                                if !workflow.extra_instructions.trim().is_empty() {
                                    workflow.extra_instructions.push_str("\n\n");
                                }
                                workflow.extra_instructions.push_str(&prompt);
    ...
    pub fn system_instructions(&self) -> String {
        let Some(runtime) = &self.runtime else {
            return self.extra_instructions.clone();
        };
        runtime.system_instructions(&self.extra_instructions)
    }
    ...
    pub async fn inspect_response(
        ...
        let extra_instructions = self.extra_instructions.as_str();
        ...
        if !completion_calls.is_empty() {
            ...
            let brief = runtime.brief.clone();
            let outcome =
                force_write_final_answer(&brief, extra_instructions, vec![reason], step + 1, io)
                    .await?;
    ...
    pub async fn on_max_steps(
        ...
        *io.usage += &response.usage;
        ...
        let brief = runtime.brief.clone();
        let outcome =
            force_write_final_answer(&brief, extra_instructions, failure_reasons, step + 2, io)
                .await?;
