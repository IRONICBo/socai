...
async fn run_session_login_preflight(
    site_id: &str,
    ...
    async fn recover_after_tool(&self, tool_name: &str, retry_number: u8) -> ToolRecoveryOutcome {
        ...
        let recovery = match replacement_guard {
            Ok(replacement_guard) => {
                if let Err(error) = run_session_login_preflight(self.site_id, &self.page).await {
...
async fn run_agent_task_background(
    ...
    let (_permit, lease, _activity, page, browser_options) = loop {
        ...
        if let Err(error) = run_session_login_preflight(site.id, &page).await {
            drop(activity);
            drop(lease);
            drop(permit);
            fail_task_before_run(
