...
fn app_site_id_for_intent(message: &str, fallback: Option<&str>) -> &'static str {
    const TIKTOK_MARKERS: &[&str] = &["tiktok.com", "vm.tiktok.com", "vt.tiktok.com", "tiktok"];
    ...
    const SITES: &[(&str, &[&str])] = &[
        ("tiktok", TIKTOK_MARKERS),
        ("dy", DOUYIN_MARKERS),
        ("xhs", XHS_MARKERS),
    ];

    let message = message.to_lowercase();
    // A follow-up can correct an earlier platform mention (for example,
    // "instead of XHS, use TikTok"). Prefer the last platform marker rather
    // than whichever platform happens to appear first in the sentence.
    if let Some((_, site_id)) = SITES
        .iter()
        .filter_map(|(site_id, markers)| {
            markers
                .iter()
                .filter_map(|marker| message.rfind(marker))
                .max()
                .map(|position| (position, *site_id))
        })
...
pub async fn agent_task_reply(
    ...
    // Persisted tasks that predate `site_id` were created by the XHS-only
    // desktop runner. Default them to XHS instead of reinterpreting old text.
    let inherited_site_id = existing.site_id.as_deref().unwrap_or(DEFAULT_APP_SITE_ID);
