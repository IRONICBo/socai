...
export namespace voiceInput {
  ...
  export function composerState(): ComposerVoiceState {
    ...
    const available = !!status?.ready && status.route === "cloud" && mediaSupported;
    let title: string;
    if (phase === "recording") title = t("voice.stop");
    else if (phase === "requesting") title = t("voice.requesting");
    else if (phase === "transcribing") title = t("voice.transcribingCloud");
    ...
    else if (status.ready) title = t("voice.start");
